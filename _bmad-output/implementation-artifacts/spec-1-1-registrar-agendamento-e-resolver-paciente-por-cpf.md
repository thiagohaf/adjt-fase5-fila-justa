...
**Problem:** `triagem-score-service` (produto FilaJusta, descartado) precisa virar `agendamento-confirmacao-service` (AD-1) antes de qualquer código novo; e não existe hoje nenhum jeito de registrar um Agendamento associado a um Paciente resolvido por CPF sem o CPF circular além do serviço de ingestão.
...
- Módulo Maven, artifactId, pacote Java (`com.filajusta.triagem` → `com.filajusta.agendamento`) e schema Postgres (`triagem_score` → `agendamento_confirmacao`) são renomeados por completo — nenhuma referência textual a `triagem`/`Triagem`/`Score` sobrevive fora de comentários de changelog/migration antiga já removida.
...
- Construto CDK novo segue o molde de `buildAuthService` (`infra-cdk/.../FilaJustaStack.java:608-674`): Fargate arm64, SG de app liberando só `sgGatewayApp`, SG de health público, rota registrada em `gateway-service/src/main/resources/application.yml`, segredos via Secrets Manager.
...
- `TriagemScoreServiceTaskRole` em `FilaJustaStack.java` não é reaproveitada com o mesmo nome/permissão (`score-calculado.fifo`) — remover ou deixar renomeação real para quando o outbox existir (Story 1.2); não criar role fantasma sem uso nesta story.
...
- `.../com/filajusta/triagem/domain/{Paciente,Cpf,CpfInvalidoException}.java` -- mover para pacote `com.filajusta.agendamento.domain`, manter lógica.
- `.../application/command/{ResolverOuCriarPaciente,PacienteRepositorio}.java` -- mover para `com.filajusta.agendamento.application.command`, manter lógica.
...
- `infra-cdk/.../FilaJustaStack.java:608-674` (`buildAuthService`) -- molde do novo `buildAgendamentoConfirmacaoService`; `:125-157` (SGs existentes) -- molde para `sgAgendamentoConfirmacaoApp`/`sgAgendamentoConfirmacaoHealth`.
- `infra-cdk/.../FilaJustaStack.java:282-292` (`TriagemScoreServiceTaskRole`) -- remover (sem outbox/SNS nesta story; não recriar role fantasma).
...
- [x] `triagem-score-service/` -- renomear diretório e artefato Maven para `agendamento-confirmacao-service`, pacote `com.filajusta.triagem` → `com.filajusta.agendamento` -- AD-1
...
- [x] `infra-cdk/.../FilaJustaStack.java` -- `buildAgendamentoConfirmacaoService` (molde `buildAuthService`), 2 SGs (app só de `sgGatewayApp`, health público), sem SG/porta gRPC -- AD-11 (gRPC adiado)
- [x] `infra-cdk/.../FilaJustaStack.java` -- remover `TriagemScoreServiceTaskRole` (sem substituto nesta story) -- evita role fantasma sem uso
...
- `mvn -pl infra-cdk -am test` (rodado da raiz do repo, não de dentro de `infra-cdk/`) -- expected: `FilaJustaStackTest` passa, incluindo a contagem de 4 `AWS::ECS::Service` e a ausência de `TriagemScoreServiceTaskRole`
...
- Agregado novo com estado inicial único e enum já preparado para as próximas transições (AD-4).
  [`Agendamento.java:19`](../../agendamento-confirmacao-service/src/main/java/com/filajusta/agendamento/domain/Agendamento.java#L19)

- Orquestração: CPF → recursoId → dataHoraAgendamento, nessa ordem determinística, antes de tocar o repositório.
  [`RegistrarAgendamento.java:49`](../../agendamento-confirmacao-service/src/main/java/com/filajusta/agendamento/application/command/RegistrarAgendamento.java#L49)

- `recursoId` validado só por formato UUID — decisão explícita de não sincronizar com o catálogo real (AD-1).
  [`RegistrarAgendamento.java:63`](../../agendamento-confirmacao-service/src/main/java/com/filajusta/agendamento/application/command/RegistrarAgendamento.java#L63)
...
- Endpoint de registro, sem Bean Validation — toda a validação vive no domínio/aplicação.
  [`AgendamentoController.java:27`](../../agendamento-confirmacao-service/src/main/java/com/filajusta/agendamento/infrastructure/web/AgendamentoController.java#L27)

- Os três `422` (CPF, recursoId, data) mapeados 1:1 para exceções de domínio/aplicação.
  [`AgendamentoExceptionHandler.java:42`](../../agendamento-confirmacao-service/src/main/java/com/filajusta/agendamento/infrastructure/web/AgendamentoExceptionHandler.java#L42)

- Fallback de erro inesperado agora loga a exceção original antes do `500` (patch do code review).
  [`AgendamentoExceptionHandler.java:75`](../../agendamento-confirmacao-service/src/main/java/com/filajusta/agendamento/infrastructure/web/AgendamentoExceptionHandler.java#L75)
...
- Entidade JPA reaproveitada do serviço antigo, só o schema muda.
  [`AgendamentoJpaEntity.java:23`](../../agendamento-confirmacao-service/src/main/java/com/filajusta/agendamento/infrastructure/persistence/AgendamentoJpaEntity.java#L23)
...
- Novo construto Fargate, molde de `buildAuthService`; sem SG/porta gRPC nesta story (adiado para Story 2.1).
  [`FilaJustaStack.java:719`](../../infra-cdk/src/main/java/com/filajusta/infra/FilaJustaStack.java#L719)

- `TriagemScoreServiceTaskRole` removida sem substituto — sem outbox/SNS nesta story.
  [`FilaJustaStack.java:270`](../../infra-cdk/src/main/java/com/filajusta/infra/FilaJustaStack.java#L270)
...
- Contagem de `AWS::ECS::Service` atualizada de 3 para 4 e teste da role antiga substituído por asserção de ausência.
  [`FilaJustaStackTest.java:44`](../../infra-cdk/src/test/java/com/filajusta/infra/FilaJustaStackTest.java#L44)
