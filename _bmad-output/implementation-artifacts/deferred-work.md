...
- source_spec: `_bmad-output/implementation-artifacts/spec-3-4-liberacao-automatica-recurso.md`
  summary: Story 3-4b (consumo da liberação agendada e liberação real do Recurso) -- `LiberacaoRecursoConsumerJob` + comando `LiberarRecurso` (idempotente por `alocacaoId`, `UPDATE` condicional `alocacao.status='ATIVA'->'LIBERADA'`), `RecursoRepositorio#marcarDisponivel`, publicação de `RecursoLiberado` via outbox/relay SNS FIFO já existente, e o teste de concorrência real (`ExecutorService`/`CountDownLatch` contra Postgres real, molde de `UltimaSugestaoRegistradaRepositorioAdapterIntegrationTest`) provando que redelivery/concorrência não duplicam a liberação nem o evento.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-3-4a1-persistencia-liberacao-agendada.md`
  summary: `LiberacaoAgendadaRepositorioAdapter#buscarPendentes` não é `@Transactional` por si só -- o `SELECT ... FOR UPDATE SKIP LOCKED` da query nativa só protege de fato contra duas instâncias do futuro relay (Story 3-4a2) lendo a MESMA linha pendente quando o CHAMADOR envolve `buscarPendentes` + `marcarComoEnviado` na mesma transação; chamado como está hoje (fora de uma transação já aberta), o lock é liberado assim que o método retorna, antes de qualquer publicação real acontecer.
  evidence: Achado convergente do blind-hunter (2 observações relacionadas: ausência de `@Transactional` cobrindo as duas chamadas, e ausência de teste de concorrência real `ExecutorService`/`CountDownLatch` provando a exclusão mútua). Nenhum consumidor chama `buscarPendentes` hoje (Boundaries da spec 3-4a1: sem relay, sem consumidor nesta story) -- não é um bug agora, mas é uma restrição de design que a Story 3-4a2 precisa respeitar explicitamente (envolver as 2 chamadas na mesma transação) e validar com um teste de concorrência real, mesmo molde de `UltimaSugestaoRegistradaRepositorioAdapterIntegrationTest`.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-4a1-persistencia-liberacao-agendada.md`
  summary: Se `Recurso.especificidadeRank` algum dia assumir um valor fora de 1-4 (hoje bloqueado só pelo dataset de seed do Epic 5, sem nenhuma validação de domínio em código que imponha esse limite), `LiberacaoDuracaoProperties#duracaoParaRank` lança `IllegalStateException` não tratada DENTRO da transação `@Transactional` de `ConfirmarAlocacao`, depois que `alocacaoRepositorio.confirmar`, `recursoRepositorio.marcarIndisponivel` e `eventoOutboxRepositorio.salvar` já rodaram -- a transação inteira sofre rollback, mas o cliente recebe um 500 genérico em vez de um erro de domínio mapeado em RFC 7807 como os demais casos desta classe (`RecursoNaoEncontradoException`, etc.).
  evidence: Achado convergente do blind-hunter e do edge-case-hunter (independentemente). Inalcançável hoje dado o design atual do domínio (nenhum código cria um Recurso com rank fora de 1-4), mas é uma lacuna latente sem guarda explícita -- a Boundaries da spec 3-4a1 só cobre o caso de falha de CONFIG (fail-fast no boot), não o caso de um Recurso com rank fora do intervalo esperado pela config.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-4a1-persistencia-liberacao-agendada.md`
  summary: `LiberacaoAgendadaJpaRepository#marcarEnviado` é um `@Modifying @Query` JPQL de update em massa sem `clearAutomatically`/`flushAutomatically` -- se uma entidade `LiberacaoAgendadaJpaEntity` já estiver no contexto de persistência (cache de 1º nível) no momento desse update, a instância em memória ficaria com `enviadoEm` desatualizado até um clear/refresh explícito.
  evidence: Achado pelo blind-hunter. Nenhum caminho atual dispara esse cenário (só os testes chamam `marcarComoEnviado`, cada um em sua própria transação, sem leitura prévia da mesma entidade na mesma sessão) -- vale documentar por que é seguro hoje ou corrigir quando a Story 3-4a2 (relay real) passar a chamar este método em produção.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-4a1-persistencia-liberacao-agendada.md`
  summary: `LiberacaoAgendadaJpaRepository#buscarPendentesParaAtualizar` não valida `limite <= 0` antes de montar a query nativa com `LIMIT :limite` -- um valor negativo faria o Postgres rejeitar a query com erro SQL ("LIMIT must not be negative").
  evidence: Achado pelo edge-case-hunter. Não alcançável hoje (só os testes chamam `buscarPendentes` com valores positivos controlados; nenhum consumidor real existe ainda) -- vale adicionar uma guarda quando a Story 3-4a2 (relay real, provável fonte de um valor de config) passar a ser a chamadora de fato.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-4a2-publicacao-liberacao-agendada.md`
  summary: `LiberacaoAgendadaRelayJob#publicarPendentes` mantém uma ÚNICA transação `@Transactional` (e a conexão/lock de linha `FOR UPDATE SKIP LOCKED`) aberta durante todo o lote (até `batch-size=50`), incluindo as chamadas de rede síncronas e bloqueantes ao `sqsClient.sendMessage` (timeout de API de 15s cada) -- uma fila SQS lenta ou indisponível pode prender a conexão do pool por minutos.
  evidence: Achado pelo blind-hunter. Padrão herdado deliberadamente de `RelaySnsPublisherJob.publicarPendentes` (Story 3-3a, já em produção) -- a spec 3-4a2 exige explicitamente "molde de RelaySnsPublisherJob" e a MESMA transação cobrindo busca+envio+marcação (Boundaries, para fechar a lacuna de concorrência da 3-4a1); não é um problema introduzido por esta story, é uma característica arquitetural já aceita e replicada.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-4a2-publicacao-liberacao-agendada.md`
  summary: `LiberacaoAgendadaRepositorio#marcarComoEnviado` é ele próprio `@Transactional` e participa (propagação `REQUIRED`) da transação do job; se lançar uma `DataAccessException` real vinda do Spring (diferente do `RuntimeException` artificial usado no teste unitário mockado), o `TransactionInterceptor` marca a transação inteira como `rollback-only` ANTES do catch local em `publicarEMarcar` executar -- o catch engole a exceção e o loop segue, mas o commit final falhará (`UnexpectedRollbackException`), desfazendo silenciosamente o `enviado_em` de outros itens do MESMO lote que já tiveram sua mensagem publicada com sucesso no SQS, causando republicação duplicada.
  evidence: Achado pelo verification-gap-reviewer/blind-hunter (convergente). Mesmo padrão de isolamento de falha por item já usado em `RelaySnsPublisherJob.publicarEMarcar` (Story 3-3a) -- a spec 3-4a2 exige explicitamente "mesmo padrão de publicarEMarcar"; nenhum teste (nem desta story nem da 3-3a) força uma falha real de `DataAccessException` dentro de uma transação Spring gerenciada para provar ou refutar o comportamento.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-4a2-publicacao-liberacao-agendada.md`
  summary: Não há teto, backoff ou alerta para falhas crônicas do LADO DA PUBLICAÇÃO -- uma linha de `liberacao_agendada` que nunca consiga ser publicada (IAM incorreto, `queue-url` malformado, SQS fora do ar) é retentada para sempre a cada ciclo de poll, sem métrica ou sinal para o operador; o `maxReceiveCount=5`/DLQ só protege contra mensagens envenenadas já dentro da fila, não contra falhas antes de a mensagem existir.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-3-4b1-liberacao-real-recurso.md`
  summary: Nenhum teste prova o comportamento de `LiberarRecurso` sob falha parcial real (ex.: `marcarDisponivel` ou `eventoOutboxRepositorio.salvar` lançando exceção depois de `alocacaoRepositorio.liberar` já ter retornado `true`) -- não há teste de rollback transacional genuíno para este comando.
  evidence: Achado pelo blind-hunter. Mesmo padrão de lacuna já presente em `ConfirmarAlocacao` (nenhum teste do serviço força rollback real via `TransactionInterceptor`) -- gap sistêmico de cobertura de teste do padrão `@Transactional` usado em todos os comandos deste serviço, não introduzido especificamente por esta story.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-4b1-liberacao-real-recurso.md`
  summary: `LiberarRecurso` confia cegamente no par `(alocacaoId, recursoId)` recebido do chamador -- nunca relê a Alocação para confirmar que `recursoId` corresponde de fato à `alocacaoId` informada; um chamador com bug ou mensagem corrompida marcaria o Recurso ERRADO como disponível enquanto o Recurso real ligado àquela Alocação permaneceria indisponível para sempre, silenciosamente.
  evidence: Achado convergente do edge-case-hunter e do blind-hunter. Inalcançável hoje (nenhum consumidor real chama `LiberarRecurso` ainda -- Story 3-4b2, deferida) -- mesma lógica de risco latente-mas-não-alcançável já aceita para o caso de `alocacaoId` inexistente (Design Notes da spec 3-4b1); a Story 3-4b2 (consumidor SQS) é quem vai de fato produzir esse par a partir da mensagem, e deve decidir se vale a pena validar.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-4b1-liberacao-real-recurso.md`
  summary: `LiberarRecurso` não loga nem emite métrica quando o UPDATE condicional não tem efeito (`alocacaoId` já `LIBERADA` ou inexistente) -- não há como o operador distinguir um replay legítimo de idempotência de um bug real do futuro chamador (recursoId errado, mensagem corrompida) só olhando os logs.
  evidence: Achado pelo blind-hunter. Mesmo padrão de ausência de observabilidade já documentado noutras entradas deste arquivo para o lado da publicação (ex.: falhas crônicas sem alerta no `LiberacaoAgendadaRelayJob`) -- gap sistêmico, não introduzido especificamente por esta story.
