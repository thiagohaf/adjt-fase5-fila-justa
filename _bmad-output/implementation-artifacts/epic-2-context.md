...
- Serviço dono: `triagem-score-service` (Clean Architecture — `domain/` sem framework → `application/command|query/` → `infrastructure/`). CQRS lógico: `RegistrarTriagem`/`ResolverOuCriarPaciente` em `command`; `ConsultarTriagem` em `query`. `ListarScoresAtuais` (bootstrap de réplica para Epic 3) também estava previsto neste pacote, mas foi explicitamente adiado para o Epic 3 pela spec 2.2 (ver `spec-2-2-consulta-triagem-score-fatores-contribuintes.md`, seção Boundaries/Never) — ainda não implementado. Mesmo banco, sem read-model separado.
