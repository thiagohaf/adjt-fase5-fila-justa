 ...
 * {@code confirmasus.matching.outbox-relay.endpoint-override} fica vazio -- o
 * SDK resolve credenciais (variavel de ambiente/role da task) e regiao pela
 * cadeia default, nunca hardcoded.
 *
 * <p>Namespace {@code confirmasus.matching.outbox-relay.*} (nao
 * {@code confirmasus.matching.relay.*}) deliberado (Boundaries da spec 3-3a):
 * este servico ja usa {@code confirmasus.matching.relay.*} para o consumidor
 * SQS de {@code ScoreCalculado} ({@link com.confirmasus.matching.infrastructure
 ...
 * {@code confirmasus.matching.outbox-relay.enabled}, default {@code true} em
 * {@code application.yml} -- mesmo padrao de {@code confirmasus.triagem.relay}
 ...
 */
@Configuration
@ConditionalOnProperty(prefix = "confirmasus.matching.outbox-relay", name = "enabled", matchIfMissing = false)
class RelaySnsClientConfig {

    private static final int API_CALL_TIMEOUT_SECONDS = 10;

    @Bean
    SnsClient snsClient(@Value("${confirmasus.matching.outbox-relay.endpoint-override:}") String endpointOverride,
                         @Value("${confirmasus.matching.outbox-relay.region:}") String region) {
