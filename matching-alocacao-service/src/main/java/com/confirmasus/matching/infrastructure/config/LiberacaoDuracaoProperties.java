package com.confirmasus.matching.infrastructure.config;
 ...
 * {@code confirmasus.liberacao.duracao.rank-{1..4}} (formato {@code Duration},
 * ex. {@code PT2M}), mesmo padrão de {@code @Value} usado por
 * {@link com.confirmasus.matching.domain.PrioridadeEfetiva} (
...
public final class LiberacaoDuracaoProperties {
    ...
    private static Duration validar(int rank, Duration duracao) {
        Objects.requireNonNull(duracao,
                () -> "confirmasus.liberacao.duracao.rank-" + rank + " nao configurado");
        ...
        if (duracao.toSeconds() < 1) {
            throw new IllegalStateException(
                    "confirmasus.liberacao.duracao.rank-" + rank + " deve ser positivo: " + duracao);
        }
        if (duracao.compareTo(LIMITE_SQS_DELAY) > 0) {
            throw new IllegalStateException(
                    "confirmasus.liberacao.duracao.rank-" + rank
        ...
        if (duracao == null) {
            throw new IllegalStateException(
                    "especificidadeRank " + especificidadeRank
                            + " sem duracao configurada em confirmasus.liberacao.duracao");
        }
        return duracao;
    }
}
