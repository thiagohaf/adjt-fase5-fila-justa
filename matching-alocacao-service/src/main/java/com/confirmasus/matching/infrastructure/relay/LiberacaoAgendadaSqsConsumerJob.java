...
import com.confirmasus.matching.infrastructure.messaging.LiberacaoAgendadaEvent;
 ...
 * ({@code confirmasus.matching.liberacao-agendada-consumer.enabled}, SEM
 ...
 * customizável via {@code confirmasus.matching.liberacao-agendada-consumer.poll-interval-ms}
...
class LiberacaoAgendadaSqsConsumerJob {
    ...
    LiberacaoAgendadaSqsConsumerJob(
            ...
            @Value("${confirmasus.matching.liberacao-agendada-consumer.queue-url}") String queueUrl,
            @Value("${confirmasus.matching.liberacao-agendada-consumer.dlq-url}") String dlqUrl,
            @Value("${confirmasus.matching.liberacao-agendada-consumer.batch-size:10}") int loteTamanho) {
        ...
        if (queueUrl == null || queueUrl.isBlank()) {
            throw new IllegalStateException(
                    "confirmasus.matching.liberacao-agendada-consumer.queue-url nao pode ser vazio com "
                            + "confirmasus.matching.liberacao-agendada-consumer.enabled=true");
        }
        if (dlqUrl == null || dlqUrl.isBlank()) {
            throw new IllegalStateException(
                    "confirmasus.matching.liberacao-agendada-consumer.dlq-url nao pode ser vazio com "
                            + "confirmasus.matching.liberacao-agendada-consumer.enabled=true");
        }
    ...
    @Scheduled(fixedDelayString = "${confirmasus.matching.liberacao-agendada-consumer.poll-interval-ms:5000}")
