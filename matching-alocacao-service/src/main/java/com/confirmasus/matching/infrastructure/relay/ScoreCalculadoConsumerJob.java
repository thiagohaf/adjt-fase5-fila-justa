 ...
 * ({@code confirmasus.matching.relay.enabled}, default {@code true}).
 */
@Component
@ConditionalOnProperty(prefix = "confirmasus.matching.relay", name = "enabled", matchIfMissing = true)
class ScoreCalculadoConsumerJob {
    ...
    ScoreCalculadoConsumerJob(SqsClient sqsClient,
                               ...
                               @Value("${confirmasus.matching.relay.queue-url}") String queueUrl,
                               @Value("${confirmasus.matching.relay.batch-size:10}") int loteTamanho,
                               @Value("${confirmasus.matching.relay.wait-time-seconds:1}") int waitTimeSeconds) {
        ...
        if (queueUrl == null || queueUrl.isBlank()) {
            throw new IllegalStateException(
                    "confirmasus.matching.relay.queue-url nao pode ser vazio com confirmasus.matching.relay.enabled=true");
        }
    ...
    @Scheduled(fixedDelayString = "${confirmasus.matching.relay.poll-interval-ms:5000}")
