...
class ScoreCalculadoConsumerJobIntegrationTest {
    ...
    static void relayProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("confirmasus.matching.relay.endpoint-override", () -> localstack.getEndpoint().toString());
        registry.add("confirmasus.matching.relay.region", localstack::getRegion);
        registry.add("confirmasus.matching.relay.queue-url", () -> queueUrl);
        registry.add("confirmasus.matching.relay.poll-interval-ms", () -> "500");
        registry.add("confirmasus.matching.relay.wait-time-seconds", () -> "1");
        // Relay outbox (Story 3-3a) e relay de liberacao agendada (Story
        // 3-4a2) desligados aqui -- este teste so cobre o consumidor SQS de
        // ScoreCalculado, sem topico SNS/fila propria no LocalStack para
        // eles; sem isso os dois falhariam ao subir (topic-arn/queue-url
        // vazios, fail-fast).
