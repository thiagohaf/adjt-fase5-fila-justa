package com.confirmasus.auth.infrastructure.security;

import com.confirmasus.auth.application.query.TokenIssuer;
...
class JwtTokenIssuer implements TokenIssuer {
    ...
    JwtTokenIssuer(@Value("${confirmasus.jwt.secret}") String secret,
                   @Value("${confirmasus.jwt.expiration-seconds}") long expirationSeconds) {
        if (expirationSeconds <= 0) {
            // exp <= iat seria um token ja expirado (ou com intervalo
            // invalido) desde a emissao -- misconfiguracao, falha rapido.
            throw new IllegalArgumentException(
                    "confirmasus.jwt.expiration-seconds deve ser positivo, recebido: " + expirationSeconds);
        }
